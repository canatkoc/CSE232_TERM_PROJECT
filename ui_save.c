...
void print(){
    ...
    while (current != -1 && count < MAX_VISIBLE) {
        ...
        mvprintw(screenRow, TEXT_START_COL, "%s", textbuffer[current].statement);
    ...
    if (head == -1) {
        mvprintw(TEXT_START_ROW, TEXT_START_COL, "(empty - use E to open or I to insert)");
    }
...
int cursorChar(){
    ...
    len = (int)strlen(textbuffer[line].statement);
...
void replace(int charIndex){
    ...
    len = (int)strlen(textbuffer[lineIndex].statement);
    if (len == 0) return;
    ...
    if (newCh != ERR && newCh >= 32 && newCh < 127) {
        textbuffer[lineIndex].statement[charIndex] = (char)newCh;
    }
}
